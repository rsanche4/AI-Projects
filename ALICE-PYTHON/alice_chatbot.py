...
# Desc: The GUI and part of the Brain of Alice
from tkinter import *
...
if os.path.isfile("brain"+os.sep+"zbrain.brn"):
    ...
    kernel.bootstrap(learnFiles = "brain"+os.sep+"mitsuku"+os.sep+"*.aiml")
    kernel.bootstrap(learnFiles = "brain"+os.sep+"extras"+os.sep+"*.aiml")
...
def alice_says(m, kernel):
    ...
    if 'can you tell the time' in m.lower() or 'tell me the time' in m.lower() or 'what time is it' in m.lower() or 'what is the time' in m.lower() or 'tell me the date' in m.lower() or 'what date is today' in m.lower() or "what's the date" in m.lower() or "what day is today" in m.lower() or "what is today's date" in m.lower() or "what's today's date" in m.lower() or "do you know the time" in m.lower():
        ...
        return da
    elif "what " in m.lower() or "when " in m.lower() or "why " in m.lower() or "who " in m.lower() or "define " in m.lower() or "definition of " in m.lower() or "meaning of " in m.lower():
        query=m+"+ wikipedia"
        search(query)
        url_list=[]
    ...
    elif m.lower()=='alright' or m.lower()=='ok' or m.lower()=='got it' or m.lower()=='yeah' or m.lower()=='lol' or m.lower()=='cool' or m.lower()=='sure' or m.lower()=='yup' or m.lower()=='yes' or m.lower()=='dont worry about it':
...
def myClick():
    ...
    if warns>=3:
        ...
        z_mes=alice_says(e.get(), kernel)
    file.write("User: "+e.get()+'\n\n')
    file.write("Alice: "+z_mes+'\n\n')
...
file.write("Alice: "+displayed_message+"\n\n")
zo = Label(root,text="ALICE CHATBOT", width=17,font=('Courier', 23, 'bold'),bg='black',fg="violet",borderwidth=10,relief="ridge")
zo.place(x=534, y=20)
